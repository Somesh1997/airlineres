...
public class ChangePassCustomerDao {
	
	public int CheckOldPassword(ChangePassCustomerBean bean) {
		...
		try {
			...
			System.out.println("count ki value"+count);
		}
		catch(Exception e) {
	...
	public int updateNewPassword(ChangePassCustomerBean bean) {
		Connection conn = null;
		int result = 0;
		try {
		conn = DBUtil.ConnectionManager();
